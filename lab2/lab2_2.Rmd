...
```{r}
US_nonUS_total_earnings_columns <-colSums(all_harry_potter_matrix)
US_nonUS_total_earnings_columns
```

2. Add this information to the data matrix. Hint: you are adding a row, not a column.  
```{r}
US_nonUS_total_earnings_matrix <- rbind(all_harry_potter_matrix, US_nonUS_total_earnings_columns)
US_nonUS_total_earnings_matrix
```
