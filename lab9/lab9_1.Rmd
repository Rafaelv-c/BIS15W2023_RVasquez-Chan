...
#Too much data, this is an example of overplotting
```
...
```{r}
ggplot(homerange, mapping=aes(x=log10.preymass, y=log10.hra))+geom_point()+
  geom_smooth(method=lm, se=T)
```
...
homerange %>% 
  filter(taxon == "mammals") %>% 
  count(trophic.guild) %>% 
  ggplot(aes(x=trophic.guild, y=n))+geom_bar(stat="identity")
```
