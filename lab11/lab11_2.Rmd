...
```{r}
my_palette2 <- paletteer_d("fishualize::Petromyzon_marinus")

homerange %>% 
  ggplot(aes(x=log10.mass, y = log10.hra, ))+ 
  geom_point()+
  scale_colour_brewer(values = my_palette2)
```
...
## Practice
1. Build a faceted plot that shows a distribution of log mass by habitat type.
```{r}
ecosphere %>% 
  ggplot(aes(x=log10_mass))+ 
  geom_density()+
...
ecosphere %>% 
  ggplot(aes(x=log10_mass))+ 
  geom_density()+
  facet_grid(~life_expectancy~habitat) #always rows then columns
```
